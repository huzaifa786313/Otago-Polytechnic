...
serverList = [ "samsojl1-app", "samsojl1-db", "samsojl1-web" ]

IMAGE = 'ubuntu-minimal-16.04-x86_64'
FLAVOUR = 'c1.c1r1'
NETWORK = 'samsojl1-net'
SUBNET = 'samsojl1-subnet'
ROUTER = 'samsojl1-rtr'
...
# ROUTER
    ...
    if conn.network.find_router('samsojl1') is None:
...
# LAUNCH     
    ...
    for serverName in serverList:
        if conn.compute.find_server(name_or_id=serverName) is None:
            ...
            if serverName == "samsojl1-web":   
                web = conn.compute.find_server("samsojl1-web")
                conn.compute.wait_for_server(server)
                conn.compute.add_floating_ip_to_server(web, floating_ip.floating_ip_address)
                print("Floating IP " + str(floating_ip.floating_ip_address) + " Applied To samsojl1-web.")
        else:
...
def destroy():
    ...
    # DESTROY NETWORK
    network = conn.network.find_network("samsojl1-net")
